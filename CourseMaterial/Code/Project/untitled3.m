...
city = ElGeneina;
figure()
plot(city.rain_org_t, city.rain_org)
hold on
plot(city.rain_org_t, city.rain_org, '*', 'Color', 'r')
legend('Interpolation', 'Meassurements')
xlabel('Time')
ylabel('Rain')
title('Rain plot')
rain = city.rain_org;
...
windowsize = 20;
a1 = 0.2; % Intial AR param estimate

for i = 21:21 % Update Ar 20 time
    ...
    at = zeros(N,1);
    Rw    = 10e-3;%10e-3                                      % Measurement noise covariance matrix, R_w. Note that Rw has the same dimension as Ry.
    Re    = eye(3)*10e-1;%10e-1                        % System noise covariance matrix, R_e. Note that Re has the same dimension as Rx_t1.
    Re = Z*Re*transpose(Z);
    Rx_t1 = eye(3)*10e-1; %correct?                             % Initial covariance matrix, V0 = R_{1|0}^{x,x}
...
present(inputModel)
[acfEst, pacfEst] = plotACFnPACF(w_t, 160, 'AR(1)' );
checkIfNormal(acfEst(2:end), 'ACF' );
checkIfNormal(pacfEst(2:end), 'PACF' );
checkIfWhite(w_t);
