...
%model_nvdi_log = log(model_nvdi - min(model_nvdi) +0.09);
validation_nvdi = nvdi(idx_validation_nvdi);
%validation_nvdi_log = log(validation_nvdi - min(model_nvdi) +0.09);
test_nvdi = nvdi(idx_test_nvdi);
%test_nvdi_log = log(test_nvdi - min(model_nvdi) +0.09);
...
plot(t_predict_val, ehat_val);
legend('ehat')
%exportgraphics(f, "7-stepValInput.png")
...
plot(t_predict_test, ehat_test);
legend('ehat')
%exportgraphics(f, "7-stepTestInput.png")
...
%title('NVDI: Validation')
%exportgraphics(f, "7stepValNVDIBJ.png")
...
legend('ehat', 'ehat naive')
%exportgraphics(f, "7stepTestNVDIBJ.png")
%title('NVDI: Test')
...
Re(1,1) = 5e-5; Re(2,2) = 5e-5; Re(4,4) = 5e-5; Re(10,10) = 5e-5; Re(13,13) = 5e-5;
...
for k0=1:length(trueParams)
    ...
    if abs(trueParams(k0)- xt(k0,end)) > min(max_5(:,2))
        ...
        max_5(index(1), 1) = k0;
        max_5(index(1), 2) = abs(trueParams(k0)- xt(k0,470));
    end
...
for k = [1 7]
    ...
    legend('ehat', 'ehat naive')
    xlabel('Time')
    exportgraphics(f, append(int2str(counter),".png"));
    counter = counter+1;
...
plotACFnPACF( ehat_1(150: end), 40, '1-step prediction using the Kalman filter', "Kalman1-step.png"); 
plotACFnPACF( ehat_7(150: end), 40, '7-step prediction using the Kalman filter', "Kalman7-step.png");
